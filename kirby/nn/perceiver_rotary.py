...
class PerceiverRotary(nn.Module):
    ...
    def forward(
        self,
        *,  # (   stacked  ) or (   chained   )
        inputs,  # (B, N_in, dim) or (N_all_in, dim) : N_in is simply the number of cells
        ...
        if self.use_random_masking_loss:
            ...
            if self.random_masking_loss_option == 'latent_dim':
                ...
                # randomly select masking_num timesteps (same count per batch)
                masking_num = int(N_latents * self.masking_rate)
        ...
        # why was this coded this way? was it trying to add a residual connection?
        for self_attn, self_ff in self.proc_layers:
        ...
        if not self.no_decoding:    
            ...
            return output_queries # final output
