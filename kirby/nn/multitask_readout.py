...
class MultitaskReadout(nn.Module):
    def __init__(
        ...
        if self.task in ['static_gratings', 'drifting_gratings', 'stimulus', 'stimulus_binary']:
    ...
    def forward(
        ...
        # print('output values is', output_values['DRIFTING_GRATINGS'].shape) # how do we insert CRE here??
        ...
        if self.use_cre_loss:
            ...
            # find rows where all values are 0 (if all values are 0, the max is 0)
            all_zeros = (max_values == 0)
            # fill rows where all values are 0 with 0, otherwise fill with the max value
        ...
        if self.task in ['movie_decoding_one', 'movie_decoding_three', 'scene_decoding']:
            ...
            fft_loss = ImprovedFFTLoss(high_freq_weight=3.0)
            ...
            if self.task in ['static_gratings', 'drifting_gratings', 'stimulus', 'stimulus_binary']:
                ...
                for decoder_id, spec in self.decoder_specs.items():
                    ...
                    if decoder_id == 'CRE_LINE':
                        # do not attach a decoder to each CRE token
                        mask = ~mask
                    ...
                    # compute loss
                    if output_values is not None and decoder_id in output_values:
                ...
                # movie_frames = movie_frames / 127.5 - 1.0
                ...
                output_tensor = []
                # check distribution of neural latents
                print("Neural latents stats:",
                    "mean:", output_latents.mean().item(),
                    "var:", output_latents.var().item(),
                ...
                raw_output = torch.stack(output_tensor, dim=1)  # ([batch, 1, 32, 64])
                if self.output_activation == 'sigmoid':
                    recon_imgs = torch.sigmoid(raw_output) * 255
                elif self.output_activation == 'clamp':
                    recon_imgs = torch.clamp(raw_output, 0, 1) * 255
                elif self.output_activation == 'tanh':
                    recon_imgs = (torch.tanh(raw_output) + 1) / 2 * 255
                else:
                    recon_imgs = torch.sigmoid(raw_output) * 255

                print("output image value range:", recon_imgs.min().item(), recon_imgs.max().item())
                print("original image value range:", movie_frames.min().item(), movie_frames.max().item())
                ...
                ### L1 variants ###
                # taskwise_loss['L1_loss'] = F.l1_loss(recon_imgs_rgb, movie_rgb) / 128 * 20000 * 0.6
                # taskwise_loss['focal_loss'] = ssim_loss(recon_imgs_rgb, movie_rgb) / 128 * 100 * 0.4
                # taskwise_loss['perceptual_loss'] = perceptual_loss(recon_imgs_rgb, movie_rgb) / (128*1000)
                # taskwise_loss['ssim_loss'] = ssim_loss(recon_imgs_rgb, movie_rgb) / 128 * 0.1
                # taskwise_loss['fft_loss'] = fft_loss(recon_imgs_rgb.float(), movie_rgb.float()) / 128 * 40000 * 0.005
                ...
                taskwise_loss['gradient_loss'] = gradient_loss(recon_imgs_rgb, movie_rgb) / 255*128*0.001
                loss = loss + taskwise_loss['perceptual_loss'] + taskwise_loss['focal_loss'] + taskwise_loss['L1_loss'] + taskwise_loss['ssim_loss'] + taskwise_loss['gradient_loss'] + 0.01 * taskwise_loss['fft_loss']

                if self.range_loss_weight > 0:
                    range_loss_fn = RangeLoss(target_min=0.0, target_max=255.0)
                    taskwise_loss['range_loss'] = range_loss_fn(recon_imgs_rgb) * self.range_loss_weight
                    loss = loss + taskwise_loss['range_loss']
