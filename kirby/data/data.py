...
class ArrayDict(object):
    ...
    def __setattr__(self, name, value):
        ...
        if not name.startswith("_"):
            ...
            first_dim = self._maybe_first_dim() # a number in the 400s or 3000s.. not sure.. sometimes None comes in (when building dataset)
            # if this suddenly changes to a combination of 0, 1, None, an error occurs. 0 is not allowed! (is it after masking??)
...
class LazyInterval(Interval):
    ...
    def slice(self, start: float, end: float, reset_origin: bool = True):
        ...
        # this leap doesn't matter whether it's [1.999] or []
        '''
